...
int main(void)
{
	...
	for (i = 0; i < 1000000; i++)
	{
		...
		sprintf(buf2, ">>>>----%016d----<<<<", i);
		pair.value = buf2;
		if (fastmap_outhandle_put(&ohandle, (fastmap_record_t*)&pair) != FASTMAP_OK && !seen_anything_but_success)
		{
	...
	for (i = 0; i < 1000000; i++)
	{
		...
		pair.key = buf;
		sprintf(buf2, ">>>>----%016d----<<<<", i);
		if (fastmap_inhandle_get(&ihandle, (fastmap_record_t*)&pair) != FASTMAP_OK && !seen_anything_but_success)
		{
			seen_anything_but_success = 1;
			diag("unable to get key: '%016d'", i);
		}

		if (strncmp(pair.value, buf2, ihandle.handle.attr.ksize) != 0 && !seen_anything_but_success)
		{
			seen_anything_but_success = 1;
